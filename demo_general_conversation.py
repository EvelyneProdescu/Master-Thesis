...
from rubric import score_session_history, summarize_rows, append_csv
...
from sic_framework.services.dialogflow.dialogflow import (
 ...
 #  basic evaluation; informed protocol 
            # example application file, tutorial and readme updated
            # in exchange an 3 outboarding  ex  ; how to introduce do this 3 assignments first : run play around test dialogs able to create new dialogs new featuresmimplement fist version of the implementation llm to do/ a 4th mii dialog 
            # limitations of the system ; what other data i need ; questions  record screen recording obs studio ; quality of implementation or conecept  what if it is not clear enough; documentation
            #  generate more data  add llm responding to user input  in the dialog system ;the goal is : directly user utterance coherent and consistent mini dialog open question with original question/ with the mini dialog/ part of the whole interanction
            # consistency in responses and compare between each QUALITY RELEVENCY -- RUBRIC GIVE SCORES ; GENERATE THE LLLM USER RESPONSES TO A CVS DATASET QUANTITATIVE ANALYSIS ground truth 
            #  more benchmarking ground truth vs generated resp
            # another llm score this compare my scores to it 
            # dealdline: friday morning at 9 am get 
            #  i get the feedback and improve the code  solutions for all the ideeas find papers for the each rubric part motivation or souce to back up the ideas
            #  A NN THAT GRAPH THAT 
            # WENDSADAY AT 11:00
            # MODAY AT 9 THETER PLAY 8:30
...
if __name__ == '__main__':
    ...
    # Rubric evaluation export for this session (personalize turns)
    try:
        rows = score_session_history(
            session_history,
            session_id=session_id,
            run_id=run_id,
            participant_id=participant_id or "",
        )
        eval_dir = abspath(join("docs", "evaluation"))
        os.makedirs(eval_dir, exist_ok=True)
        csv_path = join(eval_dir, "rubric_scores.csv")
        summary_path = join(eval_dir, "last_rubric_summary.json")
        ...
        print(f"Rubric rows written: {len(rows)} -> {csv_path}")
        print(f"Rubric summary written -> {summary_path}")
    except Exception as e:
        print(f"[WARN] Rubric export failed: {e}")
